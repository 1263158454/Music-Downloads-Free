...
**`scripts/music.py` 支持的 6 个命令：**

| 命令 | 功能 |
|:---|:---|
| `search <关键词>` | 跨 5 个源搜索歌曲 |
| `download <关键词>` | 单首下载（多源链 + 完整性校验 + 文件名清理） |
| `download-idx <N> <关键词>` | 指定序号下载（搜索结果中的第 N 条） |
...
- 使用 `scripts/music.py` 的 6 个命令（search / download / download-idx / download-playlist / check-playlist / rename）
...
4. **SSL 降级 wrapper**（不推荐）— 项目附带了 `scripts/music_ssl.py`，其使用 `ssl._create_unverified_context` 绕过证书校验。注意：**如果被拦截，降级 SSL 也只能看到拦截页面**，不能真正解决问题。真正的解决方法是绕过网络拦截，而不是绕过证书校验

> 注意：`music_ssl.py` 仅用于处理服务器证书链不完整的情况（`unable to get local issuer certificate` 类错误）。如果是公司网络主动拦截（返回 URL过滤页面），此 wrapper 也无用。
